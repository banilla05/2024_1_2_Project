...
public class GridCell
{
    ...
    public bool IsOccupied;                                 //셀이 건물로 차있는지 여주
    public GameObject Building;                             //셀이 재치된 건물 객체

    public GridCell(Vector3Int position)                   //생성자 객체가 호출 될 때
    {
        Position = position;
        IsOccupied = false;
        Building = null;
...
public class GridBuildingSystem : MonoBehaviour
{
    ...
    [SerializeField] private PlayerController playerController;      //플레이러 컨트롤러 참조
    [SerializeField] private float maxBuildDistance = 5f;           //건설 가능한 최대거리

    [SerializeField] private Grid grid;                         //그리드 선언 후 받아온다.
    private GridCell[,] cells;                                 //2차 배열로 선언 GridCell
    private Camera firstPersonCamera;                           //플레리어의 1인칭 카메라를 가져온다
    ...
    void Update()
    {
        ...
        if(lookPosition != Vector3.zero)                //포지션이 0이 아닌때
        {
    ...
    private void CreateGrid()
    {
        ...
        Vector3 gridCentor = playerController.transform.position;   //풀레이어 중심으로 그리드 생성 (원하는 위치로 변경가능)
        gridCentor.y = 0;
        transform.position = gridCentor - new Vector3(width * cellSize / 2.0f, 0, height * cellSize / 2);   //그리드 중심으로 이동시킨다

        for(int x = 0; x < width; x++)
        {
            for(int z = 0; z < height; z++)
            {
                Vector3Int cellPosition = new Vector3Int(x, 0, z);      //(0,0,0) (1,0,0), (2,0,0) ~ (10,0,0) for 문 돌리면서 배치
                ...
                CellObject.transform.SetParent(transform);                  //지금 오브젝트 하위로 설정한다
    ...
    private void PlaceBuilding(Vector3Int gridPosition)
    {
        ...
        if(!cell.IsOccupied)                                        //셀이 비어 있는 경우
        {
            Vector3 worldPosition = grid.GetCellCenterWorld(gridPosition);       //셀의 위치를 가져온다
            GameObject building = Instantiate(buildingPrefab, worldPosition, Quaternion.identity); //건물을 생성한다
    ...
    private Vector3 GetLookPosition()
    {
        if(playerController.isFirstPerson)      //플레이어가 1인칭 모드
        {
            ...
            if(Physics.Raycast(ray, out RaycastHit hitInfo, maxBuildDistance)) //설정한 ray에 물체가 잇을 경우
            {
            ...
            {
                Debug.DrawRay(ray.origin, ray.direction * maxBuildDistance, Color.white);   //Scene 창에서 하얀색 선으로 ray룰 보여준다
            }
        }
        //플레이어가 3인칭 모드
        else
        {
            ...
            Vector3 CharacterFoward = playerController.transform.forward;               //캐릭터 앞방향 선언
            Vector3 rayOrigin = characterPosition + Vector3.up * 1.5f + CharacterFoward * 0.5f;     //레이를 시작 위치 설정
            Vector3 rayDirection = (CharacterFoward - Vector3.up).normalized;                //레이를 쓸 방향 설정 후 정규화
            ...
            {
                Debug.DrawRay(ray.origin, ray.direction * hitInfo.distance, Color.blue);    //Scene 창에서 칭색 선으로 ray를 보여준다
                return hitInfo.point;                                                   //raycast 된 물체 정보를 리턴
            }
            else
            {
