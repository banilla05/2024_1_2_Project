...
{
    public PlayerState currentState;        //현재 플페이어의 상태를 나타내는 변수
    public PlayerController PlayerController;     //PlayerController를 참조

    private void Awake()
    {
        PlayerController = GetComponent<PlayerController>();        //게임 오브젝트에 붙어있는 PlayerController
    }
    ...
    void Update()
    {
        //현재 상태가 존재한다면 해당 상태의 Update 매서드 호출
    ...
    private void FixedUpdate()
    {
        ...
        //현재 상태가 존재한다면 해당 상태의 FixedUpdate 매세더 호출
        if (currentState != null)
        {
